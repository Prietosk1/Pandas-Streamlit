...
if archivo_cargado:
    vendedor=["Alirio Parra","Ivan Sepulveda","Jorge Parra","Julian Lozano","Liliana Solano","Luis Solano","Mafe Solano","Ramiro Miranda","Raul Niebles"]
    ...
    if filtro == sheet_names[0]: #consolidado 2024
        ...
        if "PORCENTAJE" in df.columns and "MES" in df.columns:
            ...
            fig.update_layout(title="Grafico del porcentaje de ventas", showlegend=True, margin={"t":40, "l":40, "r":40}, height=500)
            st.plotly_chart(fig)
        fig = px.bar(df, x="MES", y=["DISTEXTIL", "ALIPAR"], 
        ...
        df_melted = df.melt(id_vars=["MES"], value_vars=["DISTEXTIL", "ALIPAR"], 
                                var_name="Empresa", value_name="Ventas")
        ...
        if "VENTAS" in df.columns:
            st.subheader(f"Promedio de ventas {filtro} 2024")#promedio de venta por vendedor
            df["VENTAS"]= pd.to_numeric(df['VENTAS'], errors='coerce').astype(int)
        ...
        if "PORCENTAJE" in df.columns and "MES" in df.columns:
            ...
            with columna1:
                st.plotly_chart(fig)
            with columna2:
                st.write(f"El promedio de ventas del vendedor {filtro} para el año 2024 fue de {ventas_totales}")

        if "MES" in df.columns and "DISTEXTIL" in df.columns and "ALIPAR" in df.columns:#ventas por mes
        ...
        if cumplimiento<=0:
            st.success(f"El vendedor {filtro} cumplio con la meta propuesta, Supero la meta por {cumplimiento}")
        else:
            st.error(f"El vendedor {filtro}no cumplio con la meta propuesta. Le faltaron ${abs(cumplimiento)}")